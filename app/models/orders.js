...
exports.create = async function (packet_id, price, amount, order_placed, order_type, status) {
	await db.query("INSERT INTO orders(packet_id, price, amount, order_placed, order_type, status) VALUES (?, ?, ?, ?, ?, ?)", [packet_id, price, amount, order_placed, order_type, status]);
}

exports.update = async function (order_id, packet_id, price, amount, order_placed, order_type, status) {
	await db.query("UPDATE orders SET packet_id=?, price=?, amount=?, order_placed=?, order_type=?, status=? WHERE order_id=?",
